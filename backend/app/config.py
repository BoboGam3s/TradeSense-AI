...
class Config:
    ...
    # Ensure MONGO_URI contains the database name 'tradesense' if it's a typical Atlas URI
    if MONGO_URI and 'mongodb+srv' in MONGO_URI and '.mongodb.net/' in MONGO_URI and '?' in MONGO_URI:
        parts = MONGO_URI.split('?')
        if parts[0].endswith('.net/'):
            MONGO_URI = parts[0] + 'tradesense?' + parts[1]
            print(f"DEBUG: Adjusted MONGO_URI to include 'tradesense' database")
